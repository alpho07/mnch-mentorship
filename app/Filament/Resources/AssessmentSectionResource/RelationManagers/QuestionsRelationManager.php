...
class QuestionsRelationManager extends RelationManager {
    ...
    public function form(Form $form): Form {
        return $form->schema([
                            ...
                            ->tabs([
                                ...
                                ->badge(fn(Get $get) => $get('conditional_logic') ? '✓' : null)
                                ->badgeColor('success')
                                ->schema([
                                    ...
                                    ->afterStateHydrated(function (Forms\Components\Select $component, $state, $record) {
                                        if (!$record || !$record->conditional_logic) {
                                            $component->state('');
                                            return;
                                        }
                                        $logic = $record->conditional_logic;
                                    ...
                                    ->schema([
                                        Forms\Components\Grid::make(3)->schema([
                                            Forms\Components\Select::make('conditional_logic.question_code')
    ...
    public function table(Table $table): Table {
        return $table
                        ...
                        ->columns([
                            ...
                            ->label('Conditional')
                            ->state(fn(AssessmentQuestion $record): bool => !empty($record->conditional_logic))
                            ->boolean()
                            ...
                            ->falseColor('gray')
                            ->alignCenter()
                            ->tooltip(fn(AssessmentQuestion $record): ?string => !empty($record->conditional_logic) ? 'This question depends on another answer' : null
                            ),
                        ...
                        ->filters([
                            ...
                            ->query(fn($query) => $query->whereNotNull('conditional_logic')),
                        ])
                        ->headerActions([
    ...
    /**
     * Resolve the conditional_logic field from the multi-tab form before saving to DB
     */
    private function resolveConditionalLogicBeforeSave(array $data): array {
        ...
        if ($logicType === '' || $logicType === null) {
            ...
            // Already set via conditional_logic.question_code etc — keep as is
