...
<div class="container-fluid px-4">
    ...
    <div class="stats-grid">
        ...
        <!--div class="stats-card" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%);">
            ...
            <h3>{{ number_format($participants->where('completion_status', 'completed')->count()) }}</h3>
            <p>Completed</p>
        </div>
        <div class="stats-card" style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%);">
            <div class="icon"><i class="fas fa-clock"></i></div>
            <h3>{{ number_format($participants->where('completion_status', 'in_progress')->count()) }}</h3>
            <p>In Progress</p>
        </div>
                ...
                <div class="card-body">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="analytics-card">
                            ...
                            <div class="progress mb-1">
                                @php
                                    $deptRate = $stats['count'] > 0 ? round($stats['completed'] / $stats['count'] * 100, 1) : 0;
                                @endphp
                                <div class="progress-bar bg-{{ $deptRate >= 70 ? 'success' : ($deptRate >= 40 ? 'warning' : 'danger') }}" 
                                     style="width: {{ $deptRate }}%"></div>
                            </div>
                            <small class="text-muted">{{ $stats['completed'] }}/{{ $stats['count'] }} completed ({{ $deptRate }}%)</small>
                        </div>
                        @endforeach
                    </div>
                    @endif
                    ...
                    <div>
                        ...
                        <div class="analytics-card">
                            ...
                            <div class="progress mb-1">
                                @php
                                    $cadreRate = $stats['count'] > 0 ? round($stats['completed'] / $stats['count'] * 100, 1) : 0;
                                @endphp
                                <div class="progress-bar bg-{{ $cadreRate >= 70 ? 'success' : ($cadreRate >= 40 ? 'warning' : 'danger') }}" 
                                     style="width: {{ $cadreRate }}%"></div>
                            </div>
                            <small class="text-muted">{{ $stats['completed'] }}/{{ $stats['count'] }} completed ({{ $cadreRate }}%)</small>
                        </div>
                        @endforeach
                    </div>
                    @endif
