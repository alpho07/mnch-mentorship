        ...
        return $table
                        ->query(
                                ...
                                ->when(!$isAdmin, fn(Builder $q) =>
                                        $q->whereHas('training', fn($t) =>
                                                $t->where('mentor_id', $user->id)
                                        )
                                )
                                ->when($this->filterTrainingId, fn($q) =>
                                        $q->where('training_id', $this->filterTrainingId)
                                )
                                ->when($this->filterFacilityId, fn($q) =>
                                        $q->whereHas('training', fn($t) =>
                                                $t->where('facility_id', $this->filterFacilityId)
                                        )
                                )
                        ...
                        ->columns([
                            ...
                            ->label('Mentees')
                            ->getStateUsing(fn(MentorshipClass $r) =>
                                    ClassParticipant::where('mentorship_class_id', $r->id)->count()
                            )
                            ->badge()->color('primary'),
                            Tables\Columns\TextColumn::make('completed_count')
                            ->label('Completed')
                            ->getStateUsing(fn(MentorshipClass $r) =>
                                    ClassParticipant::where('mentorship_class_id', $r->id)
                        ...
                        ->filters([
                            ...
                            ->query(fn(Builder $q, array $data) =>
                                    $data['value'] ? $q->where('training_id', $data['value']) : $q
                            ),
                            ...
                            ->query(fn(Builder $q, array $data) =>
                                    $data['value'] ? $q->whereHas('training', fn($t) => $t->where('facility_id', $data['value'])) : $q
                            ),
                        ])
                        ->actions([
