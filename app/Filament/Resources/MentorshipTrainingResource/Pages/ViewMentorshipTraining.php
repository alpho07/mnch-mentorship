...
class ViewMentorshipTraining extends ViewRecord
{
                        ...
                        Grid::make(2)
                            ->schema([
                                TextEntry::make('mentor_info')
                                    ...
                                    ->getStateUsing(fn ($record) => 
                                        $record->mentor 
                                            ? "{$record->mentor->first_name} {$record->mentor->last_name}" .
                                              ($record->mentor->cadre ? " ({$record->mentor->cadre->name})" : "")
                                            : 'Not assigned'
                                    )
                                    ...
                                    ->getStateUsing(fn ($record) => 
                                        $record->organizer 
                                            ? "{$record->organizer->first_name} {$record->organizer->last_name}" .
                                              ($record->organizer->department ? " ({$record->organizer->department->name})" : "")
                                            : 'Not assigned'
                                    )
                                ...
                                Grid::make(5)
                                    ->schema([
                                        ...
                                        TextEntry::make('usage_percentage')
                                            ->label('Usage %')
                                            ->getStateUsing(fn ($record) => 
                                                $record->quantity_planned > 0 
                                                    ? round(($record->quantity_used / $record->quantity_planned) * 100, 1) . '%'
                                                    : '0%'
                                            )
                                            ->badge(),
                                    ]),
    ...
    {
        if ($material->quantity_planned <= 0) return 'Unknown';
        ...
        if ($usagePercent <= 110) return 'As Planned';
        return 'Overused';
    }
