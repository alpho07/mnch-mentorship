...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::transaction(function () use ($curriculum): void {
            $this->assertSchema();
            $this->repairModulePrefixes();
            $this->repairModuleDurations($curriculum);
            $this->repairKnownReleaseIssues();
            $this->assertClean($curriculum);
        });
    }

    public function down(): void
    {
        // Guard migration only.
    }

    private function loadCurriculum(): array
    {
    ...
    private function assertClean(array $curriculum): void
    {
        $audit = $this->audit($curriculum);
    ...
    private function repairModuleDurations(array $curriculum): void
    {
        $programs = [
        ...
        foreach ($programs as $programName => $modules) {
            ...
            foreach ($modules as $index => $moduleData) {
                $moduleRow = $activeModules[$index] ?? null;
                if (!$moduleRow) {
                    continue;
                }

                $expectedMinutes = (int) collect($moduleData['sessions'] ?? [])->sum('time_minutes');
                if ((int) $moduleRow->total_time_minutes !== $expectedMinutes) {
                    DB::table('program_modules')
                        ->where('id', $moduleRow->id)
                        ->update(['total_time_minutes' => $expectedMinutes]);
                }
            }
        }
    }

    private function repairModulePrefixes(): void
    {
        $modules = DB::table('program_modules')
            ->whereIn('program_id', $this->targetProgramIds())
            ->where('is_active', true)
            ->orderBy('order_sequence')
        ...
        foreach ($modules as $module) {
            ...
            $baseName = preg_replace('/^Module\\s+\\d+\\s*:\\s*/i', '', $name) ?? $name;
    ...
    private function repairKnownReleaseIssues(): void
    {
    ...
    private function audit(array $curriculum): array
    {
        $programAudits = [];
        $programProblems = [];
        ...
        foreach ([
            ...
            if (!$program) {
                ...
                $programProblems[] = "Program not found: {$programName}";
                continue;
            }
            ...
            $programSpecificProblems = [];
            $moduleAudits = [];

            if ($activeModules->count() !== count($modules)) {
                $problem = "Program {$programName} active module count mismatch: expected " . count($modules) . ", found " . $activeModules->count();
                $programSpecificProblems[] = $problem;
                $programProblems[] = $problem;
            }
            ...
            foreach ($modules as $index => $module) {
                ...
                if (!$row) {
                    $problem = "Program {$programName} missing active module at position {$expectedOrder}.";
                    $programSpecificProblems[] = $problem;
                    $programProblems[] = $problem;
                    $moduleAudits[] = [
                ...
                if ($moduleProblems !== []) {
                    $programSpecificProblems[] = "{$row->name}: " . implode('; ', $moduleProblems);
            ...
            $programAudits[] = [
                ...
                'status' => $programSpecificProblems === [] ? 'ok' : 'problem',
                'problems' => $programSpecificProblems,
        ...
        return [
            ...
            'problem_count' => count($programProblems) + count($referenceAudit['problems']),
    ...
    {
        $programIds = $this->targetProgramIds();
    ...
    private function assertSchema(): void
    {
        ...
        $requiredColumns = [
            'program_modules' => ['program_id', 'name', 'order_sequence', 'is_active', 'total_time_minutes'],
            'module_sessions' => ['program_module_id', 'name', 'time_minutes', 'order_sequence', 'is_active'],
            'class_modules' => ['program_module_id', 'status'],
            'class_sessions' => ['class_module_id', 'module_session_id', 'session_number', 'title', 'duration_minutes'],
