...
<body class="bg-gray-50">
    ...
    <div class="min-h-screen py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="flex items-center justify-between flex-wrap gap-4">
                            ...
                            <div class="flex items-center space-x-3">
                                @if ($mapData['summary']['avg_participants_per_training'] > 0)
                                    <div
                                        class="text-sm text-gray-600 bg-white px-4 py-2 rounded-full border shadow-sm">
                                        📊 Avg. {{ $mapData['summary']['avg_participants_per_training'] }} participants
                                        per training
                                    </div>
                                @endif
            ...
            <div id="summary-table-{{ $widgetId }}"
                class="hidden bg-white rounded-lg shadow-sm mt-6 border border-gray-200">
                <div class="p-6 border-b border-gray-200">
                    <div class="flex items-center justify-between flex-wrap gap-4">
                        <div>
                                ...
                                <tr class="county-row" data-county="{{ $county['name'] }}">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900">{{ $county['name'] }}
                                                </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-lg font-semibold text-gray-900">
                                            {{ number_format($county['participants']) }}</div>
                                        <div class="text-sm text-gray-500">Total participants</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-lg font-semibold text-gray-900">{{ $county['facilities'] }}
                                        </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <span
                                                class="text-sm font-semibold text-gray-900">{{ round($county['intensity']) }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @php
                                        ...
                                        @endphp
                                        <span
                                            class="inline-flex px-3 py-1 text-xs font-semibold rounded-full {{ $badgeClass }}">
                                            {{ $coverage }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                        @if ($county['trainings'] > 0)
                                            <button
                                                onclick="showCountyDetails('{{ $widgetId }}', '{{ $county['name'] }}', @json($county))"
                                                class="text-blue-600 hover:text-blue-900 hover:underline">View
                                                Details</button>
                                        @endif
                                        <button
                                            onclick="highlightCountyOnMap('{{ $widgetId }}', '{{ $county['name'] }}')"
    ...
    <div id="county-details-modal-{{ $widgetId }}"
        class="hidden fixed inset-0 modal-overlay overflow-y-auto h-full w-full">
        <div
            class="relative top-4 mx-auto p-6 border w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 shadow-xl rounded-xl bg-white max-h-screen overflow-y-auto fade-in">
    ...
    <script>
        ...
        function exportTableData(widgetId) {
            ...
            rows.forEach(row => {
                if (row.style.display !== 'none') {
                    ...
                    csvContent +=
                        `"${county}","${trainings}","${participants}","${facilities}","${intensity}","${coverage}"\n`;
                }
            });
        ...
        function showToast(message, type = 'info') {
            ...
            toast.className =
                `${colors[type]} text-white px-4 py-3 rounded-lg shadow-lg fade-in flex items-center space-x-2`;
