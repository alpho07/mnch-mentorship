      ...
      <div style={{ background: "linear-gradient(135deg, #0F172A 0%, #1E293B 100%)", padding: "52px 20px 20px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
          {["all", "completed", "in_progress"].map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{ padding: "6px 13px", borderRadius: 20, background: filter === f ? "white" : "rgba(255,255,255,0.1)", color: filter === f ? "#0F172A" : "rgba(255,255,255,0.7)", border: "none", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
      ...
      <div style={{ padding: "14px 16px 100px" }}>
        ...
        {!loading && list.length === 0 && (
          <div style={{ textAlign: "center", padding: "40px 20px", color: T.textMuted }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 600 }}>No assessments found</div>
          </div>
        )}
        {list.map(a => (
          <button key={a.id} onClick={() => onView(a)} style={{ width: "100%", background: T.card, borderRadius: T.radius, padding: "14px 16px", marginBottom: 10, border: "2px solid " + (a.overall_grade ? GRADE_BG[a.overall_grade] : T.borderLight), cursor: "pointer", textAlign: "left", boxShadow: T.shadow }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 700, fontSize: 14, color: T.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.facility_name}</div>
                <div style={{ fontSize: 12, color: T.textSub, marginTop: 3 }}>{a.assessment_type} · {a.assessment_date}</div>
              </div>
              <div style={{ marginLeft: 10, flexShrink: 0 }}>
                {a.overall_grade ? <GradeBadge grade={a.overall_grade} pct={a.overall_percentage} /> : <StatusChip status={a.status} />}
              </div>
            </div>
            {a.status === "completed" && sections && (
