                            ...
                            ->schema([
                                ...
                                Forms\Components\Grid::make(3)->schema([
                                    ...
                                    ->afterStateHydrated(function (Forms\Components\Select $component, $record) {
                                        if ($record && !empty($record->conditional_logic['question_code'])) {
                                            $component->state($record->conditional_logic['question_code']);
                                        }
                                    }),
                                ...
                                ->afterStateHydrated(function (Forms\Components\Textarea $component, $record) {
                                    if ($record && $record->conditional_logic) {
                                        $component->state(json_encode($record->conditional_logic, JSON_PRETTY_PRINT));
                                    }
                                })
                                ->columnSpanFull(),
                            ]),
    ...
    public static function table(Table $table): Table {
        return $table
                        ->columns([
                            ...
                            ->label('Cond.')
                            ->state(fn(AssessmentQuestion $r): bool => !empty($r->conditional_logic))
                            ->boolean()
                            ...
                            ->falseColor('gray')
                            ->alignCenter()
                            ->tooltip(fn(AssessmentQuestion $r): ?string => !empty($r->conditional_logic) ? 'Parent: ' . ($r->conditional_logic['question_code'] ?? 'multi-condition') : null
                            ),
                        ...
                        ->actions([
                            ...
                            ->color('warning')
                            ->visible(fn(AssessmentQuestion $r) => !empty($r->conditional_logic))
