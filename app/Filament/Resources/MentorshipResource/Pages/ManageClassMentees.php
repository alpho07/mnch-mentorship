        ...
        return $table
                        ...
                        ->columns([
                            ...
                            ->getStateUsing(fn(ClassParticipant $record) =>
                                    $record->invitation_sent_at ? $record->invitation_sent_at->diffForHumans() : 'Not sent'
                            )
                            ->icon(fn(ClassParticipant $record) =>
                                    $record->invitation_sent_at ? 'heroicon-o-envelope' : 'heroicon-o-envelope-open'
                            )
                            ->color(fn(ClassParticipant $record) =>
                                    $record->invitation_sent_at ? 'success' : 'gray'
                            )
                            ->toggleable()
                        ])
                        ->actions([
                            ...
                            ->label(fn(ClassParticipant $record) =>
                                    $record->invitation_sent_at ? 'Resend Invite' : 'Send Invite'
                            )
                            ->icon('heroicon-o-paper-airplane')
                            ->color(fn(ClassParticipant $record) =>
                                    $record->invitation_sent_at ? 'gray' : 'info'
                            )
                            ->visible(fn(ClassParticipant $record) => !empty($record->user->email))
                            ->requiresConfirmation()
                            ->modalHeading(fn(ClassParticipant $record) =>
                                    $record->invitation_sent_at ? 'Resend Enrollment Invitation?' : 'Send Enrollment Invitation?'
                            )
